...
test.describe(`Navigate to the ${defineConfig.use?.baseURL}/account to Testing`, () => {
  ...
  test("TC40: Pay electric bill with valid amount", async ({ page }) => {
    const data = testcase.TC40.data;
    const step = testcase.TC40.step;
    const expectation = testcase.TC40.expectation;
    const helper = new BillHelper(page);
    const balance = 8000;
    const authHelper = new AuthHelper();
  ...
  test("TC41: Pay phone bill with valid amount", async ({ page }) => {
    const balance = 2000;
    const data = testcase.TC41.data;
    const step = testcase.TC41.step;
    const expectation = testcase.TC41.expectation;
    const helper = new BillHelper(page);
    const authHelper = new AuthHelper();
